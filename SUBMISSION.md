...
1. A wallet-free way in. Telegram for people. For agents, the same call as one hosted MCP URL (one command in Claude Code or Cursor, and listed on the official MCP registry as io.github.Harshyadav442277/telegraph-morse) or one REST endpoint, with a free key that carries a daily cap. Telegraph's own MCP server needs a funded private key; Morse's needs nothing.
