...
export function proofPage(r: Reconciliation, stats: { okCalls: number; miners: number; usersAnswered: number }): string {
  ...
  const verdict = r.error
    ...
    : `<div class="panel ${allMatched ? "hero" : "warn"}"><b>${allMatched ? `All ${r.matched} distinct settlement hashes in the ledger are on chain` : `${r.matched} distinct settlement hashes are on chain; ${r.ledgerOnly.length} ledger row${r.ledgerOnly.length === 1 ? " is" : "s are"} not yet visible to the indexer (listed below)`}</b> — ${r.ledger.withSettlement} ledger rows carry a hash${r.ledger.withSettlement > r.matched + r.ledgerOnly.length ? ", and a few rows share one settlement, so hashes are fewer than rows" : ""}.${allMatched ? " Every paid call Morse has recorded is a real USDC transfer to Telegraph." : ""}${r.chainOnly.length ? ` The chain also shows <b>${r.chainOnly.length}</b> settlement${r.chainOnly.length === 1 ? "" : "s"} with no ledger row: payments that landed after Morse had already recorded the call as failed or timed out. They are counted here and not hidden.` : ""}</div>`;
