...
<p class="muted">Every call Morse makes, newest first. <b>What you ask is public:</b> the question is not in this table, but it is returned by <a href="/api/recent">/api/recent</a> clipped to 200 characters — do not type anything private. Users are salted hashes and are not shown; "people answered" counts distinct identities that received at least one answer, "who asked" includes attempts that failed. Both include the operator's own testing — this project does not separate its own calls out, and does not manufacture any. Payer wallet: ${d.payer ? `<a href="https://sepolia.basescan.org/address/${h(d.payer)}"><code>${h(d.payer)}</code></a>` : "<i>not configured</i>"}. Cross-check any row at its verify link, or on the node with <code>GET /engine/v1/signal/{hash}</code>.</p>
