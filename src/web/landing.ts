...
export function landingPage(d: LandingData): string {
  ...
  const groups = d.groups
    ...
    .join("");
  const body = `
<p class="lede">Ask a question in plain language. Telegraph routes it to a ranked miner, Morse pays the fee, and you get the answer <b>with a receipt</b> you can verify on the node and on-chain. Then <b>ask the podium</b>: the other top-ranked miners answer the same question, side by side.</p>
...
<details><summary>What does "Ask the podium" do?</summary><p>After an answer, it asks the other top-ranked miners for that intent the same question, directly, and shows all the answers with their ranks and receipts. When the answers are verdicts (valid/invalid, safe/unsafe, true/false) or figures (a price, a temperature, a balance), Morse compares them and says whether they agree, with the tolerance it used. For free-text answers it shows them side by side and says agreement was not judged. Each extra answer is a normal paid call with its own receipt.</p></details>
...
<p>Your question goes to Telegraph's own router (<code>POST /engine/v1/ask</code>), which classifies it into one of the network's 45 canonical intents with an LLM and picks a ranked miner — 70% of traffic goes to #1, 20% to #2, 10% to #3, re-ranked from validator scores every 9-hour epoch. If the router does not answer within 20 seconds (on 2 September it timed out at ~47 s for a day), Morse falls back to its own keyword classifier over the live leaderboard and calls the best-ranked miner directly. Every receipt says which of the two happened. <b>Ask the podium</b> is the optional layer on top: it never replaces routing, it only re-asks the other top-ranked miners after an answer exists, at your request.</p>
<p><b>Not an aggregator.</b> Morse does not pick providers and does not blend answers. Telegraph's ranked router picks the miner; Morse pays, keeps the receipt, and lets you hold that answer against the other ranked miners. What it adds is verification, in public: <a href="/proof">every payment reconciled against the chain</a> and <a href="/consensus">every podium round scored for agreement</a>.</p>
...
out.addEventListener('click',async e=>{const b=e.target.closest('[data-pd]');if(!b)return;b.disabled=true;b.textContent='Asking the other top-ranked miners… (up to ~30 s)';try{const r=await fetch('/api/podium',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({hash:b.dataset.pd})});const j=await r.json();const d=document.createElement('div');d.innerHTML=podium(j);b.replaceWith(d)}catch(err){b.textContent=String(err.message)}});
</script>`;
