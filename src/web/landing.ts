...
function receipt(r){if(!r)return'';const v=r.signalHash?' · <a href="/verify/'+r.signalHash+'">verify <code>'+r.signalHash.slice(0,10)+'…</code></a>':'';const c=r.confidence===null?'confidence not reported':(r.confidenceIsRisk?'risk ':'confidence ')+Math.round(r.confidence*100)+'%'+(r.confidenceIsRisk?' (miner metric)':'');return'<div class="receipt">served by <b>'+esc(r.minerSlug||'?')+'</b>'+(r.minerRank?' #'+r.minerRank:'')+(r.intent?' for '+esc(r.intent):'')+' · '+c+' · '+(r.costUsd!=null?'$'+r.costUsd.toFixed(2):'')+' · '+(r.durationMs||'')+' ms'+v+'</div>'}
