...
- [ ] Deployed (first deploy broken, fix committed, redeploy pending); **done when** a stranger's message in Telegram returns a live answer with a
      `signal_hash` that resolves at `/verify/{hash}`
- [ ] X post 1

## P2 — Web: landing, ledger, verify (2026-09-03) [A, D, T]
- [x] `/` landing: what Morse is, ask box, live counters (users, calls, intents, miners, spent),
...
- [x] `/api/stats` JSON for X screenshots and for judges
...
- [x] Recipes: `safe`, `wallet`, `weather`, `fact` (bot commands + web chips + MCP tools) — built, unmeasured on real miners
...
- [x] `/mcp` Streamable HTTP MCP server; `POST /v1/ask` REST (verified locally)
